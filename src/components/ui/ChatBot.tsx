...
type Screen = "home" | "consult";
type ConsultStep = "service" | "duration" | "budget" | "reference" | "contact" | "submitting" | "done";
...
const DURATION_OPTS = ["15초 이하", "30초", "1분", "2~3분", "5분 이상", "미정"];
...
const CONSULT_STEPS: ConsultStep[] = ["service", "duration", "budget", "reference", "contact"];

// ─── Animation variants (identical to working version) ────────────────────────
...
// ─── TypingText (identical to working version) ────────────────────────────────
...
export function ChatBot() {
  ...
  function resetAll() {
    ...
    setTypingDone(true);
    setCStep("service");
    setService(""); setDuration(""); setBudget("");
    setReference(""); setName(""); setEmail("");
    setCError(""); setSubmitting(false);
  }
  ...
  function pickService(v: string) { setService(v); setTimeout(() => setCStep("duration"), 120); }
  ...
  function submitConsult() {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setCError("올바른 이메일 형식이 아닙니다."); return; }
            ...
            <div className="flex flex-none items-center justify-between border-b border-white/[0.07] px-5 py-4">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-accent text-[9px] font-bold tracking-wide text-white">U</div>
              ...
              <div className="flex items-center gap-0.5">
                {(screen === "consult" || history.length > 0) && (
                  <button onClick={resetAll} aria-label="처음으로" title="처음으로"
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain">
              ...
              {screen === "home" && (
                <div className="px-5 py-5 space-y-4">
                  {/* Greeting */}
                  <motion.div initial="hidden" animate="show" variants={msgVariants} className="flex gap-3">
                    <BotDot />
                  ...
                  {/* Consult CTA — first, always visible */}
                  <motion.div
                  ...
                  >
                    <button onClick={goConsult}
                      className="group flex w-full items-center justify-between rounded-xl border border-brand-accent/25 bg-brand-accent/[0.07] px-4 py-3.5 text-left transition-all hover:border-brand-accent/45 hover:bg-brand-accent/[0.11]">
                      <div>
                        <p className="text-[13px] font-semibold text-white/85 transition-colors group-hover:text-white">프로젝트 상담 시작하기</p>
                        <p className="mt-0.5 text-[10.5px] text-white/35">영상 유형 · 길이 · 예산 · 레퍼런스 → 이메일 접수</p>
                      </div>
                      <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.7} className="h-4 w-4 flex-none text-brand-accent/40 transition-colors group-hover:text-brand-accent">
                  ...
                  {history.map((qid, i) => {
                    ...
                    return (
                      <motion.div key={i} initial="hidden" animate="show" variants={msgVariants} className="space-y-4">
                        ...
                        <div className="flex gap-3">
                          ...
                          <div className="flex-1 space-y-3 pt-0.5">
                            <p className="text-[13px] leading-[1.8] text-white/70">
                              {isLatest
                                ? <TypingText text={item.answer} onDone={() => setTypingDone(true)} />
                                : item.answer}
                            </p>
                            {(!isLatest || typingDone) && (
                  ...
                  <AnimatePresence>
                    {typingDone && (
                      <motion.div key="opts" initial="hidden" animate="show"
                        exit={{ opacity: 0, transition: { duration: 0.1 } }} variants={listVariants} className="pt-1">
                        {currentOptions.map((qid) => {
                          const item = QNA[qid];
                          if (!item) return null;
                          return (
                            <motion.button key={qid} variants={listItemVariants} onClick={() => faqSelect(qid)}
                              className="group flex w-full items-center justify-between gap-3 border-b border-white/[0.06] py-2.5 text-left last:border-0">
                              <span className="text-[12px] leading-snug text-white/40 transition-colors duration-200 group-hover:text-white/80">{item.question}</span>
              ...
              {screen === "consult" && (
                <div className="flex h-full flex-col px-5 py-5">
                  ...
                  {cStep === "service" && (
                    <div>
                      <p className="font-mono text-[9.5px] uppercase tracking-[0.22em] text-brand-accent/55">01</p>
                      <p className="mt-1 mb-4 text-[15px] font-medium text-white">어떤 영상을 만들고 싶으신가요?</p>
                      <div className="grid grid-cols-3 gap-2">
                        {SERVICE_OPTS.map((o) => (
                          <button key={o.label} onClick={() => pickService(o.label)}
                            className={`flex flex-col gap-1.5 rounded-xl border px-3 py-3 text-left transition-all duration-150 ${service === o.label ? "border-brand-accent bg-brand-accent/10 text-white" : "border-white/10 bg-white/[0.025] text-white/50 hover:border-white/20 hover:text-white"}`}>
                            <span className="text-sm leading-none text-brand-accent/70">{o.icon}</span>
                            <span className="text-[11.5px] font-medium leading-snug">{o.label}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  {cStep === "duration" && (
                    <div>
                      <p className="font-mono text-[9.5px] uppercase tracking-[0.22em] text-brand-accent/55">02</p>
                      <p className="mt-1 mb-4 text-[15px] font-medium text-white">영상 길이는 어느 정도인가요?</p>
                      <div className="grid grid-cols-3 gap-2">
                        {DURATION_OPTS.map((o) => (
                          <button key={o} onClick={() => pickDuration(o)}
                            className={`rounded-xl border px-3 py-3 text-left text-[11.5px] font-medium transition-all duration-150 ${duration === o ? "border-brand-accent bg-brand-accent/10 text-white" : "border-white/10 bg-white/[0.025] text-white/50 hover:border-white/20 hover:text-white"}`}>
                            {o}
                          </button>
                        ))}
                      </div>
                  ...
                  {cStep === "budget" && (
                    <div>
                      <p className="font-mono text-[9.5px] uppercase tracking-[0.22em] text-brand-accent/55">03</p>
                      <p className="mt-1 mb-4 text-[15px] font-medium text-white">예산 범위는 어느 정도인가요?</p>
                      <div className="flex flex-col gap-2">
                        {BUDGET_OPTS.map((o) => (
                          <button key={o} onClick={() => pickBudget(o)}
                            className={`flex items-center justify-between rounded-xl border px-4 py-3 text-left transition-all duration-150 ${budget === o ? "border-brand-accent bg-brand-accent/10 text-white" : "border-white/10 bg-white/[0.025] text-white/50 hover:border-white/20 hover:text-white"}`}>
                            <span className="text-[12px] font-medium">{o}</span>
                            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.7} className={`h-3.5 w-3.5 ${budget === o ? "text-brand-accent" : "text-white/18"}`}>
                              <path d="M6 4l4 4-4 4" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                          </button>
                        ))}
                  ...
                  {cStep === "reference" && (
                    <div>
                      <p className="font-mono text-[9.5px] uppercase tracking-[0.22em] text-brand-accent/55">04</p>
                      <p className="mt-1 text-[15px] font-medium text-white">레퍼런스 링크가 있으신가요?</p>
                      <p className="mt-0.5 mb-4 text-[11px] text-white/35">YouTube URL, 키워드 등 — 선택 사항입니다.</p>
                      <div className="relative border-b border-white/20">
                        ...
                        <span className="pointer-events-none absolute bottom-0 left-0 h-px w-0 bg-brand-accent transition-[width] duration-700 peer-focus:w-full" />
                      </div>
                      <div className="mt-5 flex gap-3">
                        ...
                        <button onClick={() => { setReference(""); setCStep("contact"); }}
                  ...
                  {cStep === "contact" && (
                    <div>
                      <p className="font-mono text-[9.5px] uppercase tracking-[0.22em] text-brand-accent/55">05</p>
                      <p className="mt-1 text-[15px] font-medium text-white">연락처를 남겨주세요.</p>
                      <p className="mt-0.5 mb-5 text-[11px] text-white/35">빠른 시일 내에 담당자가 연락드립니다.</p>
                      <div className="space-y-5">
                        <LineInput label="이름 *" placeholder="Name" value={name} onChange={setName} />
                        <LineInput label="이메일 *" placeholder="you@email.com" type="email" value={email} onChange={setEmail} />
                      </div>
                  ...
                  {cStep === "done" && (
                    <div>
                      ...
                      <a href={KAKAO_URL} target="_blank" rel="noopener noreferrer"
                        className="mt-4 inline-flex items-center gap-2 rounded-lg border border-amber-300/20 bg-amber-300/[0.05] px-3 py-2 text-[11px] font-medium text-amber-300/65 transition-all hover:border-amber-300/35">
                        <KakaoIcon />카카오 채널로 추가 문의하기
                      </a>
                      <div className="mt-5 border-t border-white/[0.07] pt-4">
                        <p className="mb-2 text-[9px] uppercase tracking-[0.2em] text-white/28">포트폴리오 둘러보기</p>
